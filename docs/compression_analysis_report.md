...
詳細: [`docs/phase_5.8_test_plan.md`](./phase_5.8_test_plan.md)、[`docs/Implementation/issue_tpm_embed_truncation.md`](./Implementation/issue_tpm_embed_truncation.md)
...
- [ ] CHANGELOG.md 作成
- [ ] **`topics` フィールド導入**: `tags`（システムライフサイクル管理）と `topics`（意味的内容タグ、CJK 多言語対応）を分離。`ep-save` ツールの手動保存引数を `tags` → `topics` に変更。HealingWorker による自動 topics 生成・ファセット検索対応。詳細: [`docs/phase_6_topics_plan.md`](./phase_6_topics_plan.md)
...
> [!NOTE]
> Phase 5.4 の監査で `[:16]`（=32bit、約65K件で50%衝突）の脆弱性が指摘され、**MD5全長（128bit）** に拡張済み。詳細は `docs/Implementation/api-rpm-optimization/ep_api_rpm_optimization_safe_execution_report.md` を参照。
...
3. **👻 幽霊ファイルの許容と自己修復 (Ghost File Tolerance + Self-Healing):**
   ...
   - 詳細は `docs/Implementation/api-rpm-optimization/ep_api_rpm_phase2_healing_plan.md` を参照。
...
本番環境（WSL等）での本格稼働やE2Eテストに向け、TypeScript層とGoサイドカー層のインタフェースで発生した複数の論理バグや概念のミスマッチを修正し、アーキテクチャの正確な同期を確立した。詳細は `docs/Implementation/phase-5.4/` を参照。
...
Phase 5の最終関門として、実稼働時の例外（APIエラーや初期化順序の逆転、複数エージェント同居）で発生するEdge Caseを完全に制圧した。詳細は `docs/Implementation/phase-5.5/` を参照。
...
### 18.4 ツールAPIリファクタリング
`ep-save` / `ep-recall` / `ep-expand` の全ツールをOpenClaw正式API仕様に準拠（TypeBoxスキーマ、`execute(_toolCallId, params)` シグネチャ、`{ content: [{ type: "text", text }] }` 返却形式）。詳細は `docs/Implementation/ep-tools-refactor/` を参照。

### 18.5 `saved_by` フィールド修復
`ep-save` の `saved_by` が常に `"auto"` になる問題を、`extractAgentId()` ヘルパーとバケツリレー設計で解決。Go側の `handleIngest` / `handleBatchIngest` の対称性も確保。詳細は `docs/Implementation/ep-save-fix/` を参照。

詳細レポート: `docs/Implementation/cli-blocker/episodic_claw_cli_blocker_report.md`
...
| ラウンド | 日付 | 新規発見 | ファイル |
|----------|------|----------|---------|
| Round 1 | 2026-03-25 | 6件（HIGH×2, MED×2, LOW×2） | [`docs/Implementation/compression-audits/compression_analysis_audit_round1.md`](./Implementation/compression-audits/compression_analysis_audit_round1.md) |
| Round 2 | 2026-03-25 | 11件（HIGH×3, MED×4, LOW×4） | [`docs/Implementation/compression-audits/compression_analysis_audit_round2.md`](./Implementation/compression-audits/compression_analysis_audit_round2.md) |
| Round 3 | 2026-03-25 | 5件（MED×3, LOW×2） | [`docs/Implementation/compression-audits/compression_analysis_audit_round3.md`](./Implementation/compression-audits/compression_analysis_audit_round3.md) |
| Round 4 | 2026-03-25 | 3件（MED×2, LOW×1） | [`docs/Implementation/compression-audits/compression_analysis_audit_round4.md`](./Implementation/compression-audits/compression_analysis_audit_round4.md) |
| Round 5 | 2026-03-25 | 3件（MED×2, LOW×1） | [`docs/Implementation/compression-audits/compression_analysis_audit_round5.md`](./Implementation/compression-audits/compression_analysis_audit_round5.md) |
| Round 6 | 2026-03-25 | 3件（MED×2, LOW×1） | [`docs/Implementation/compression-audits/compression_analysis_audit_round6.md`](./Implementation/compression-audits/compression_analysis_audit_round6.md) |
| Round 7 | 2026-03-25 | 収束 | [`docs/Implementation/compression-audits/compression_analysis_audit_round7.md`](./Implementation/compression-audits/compression_analysis_audit_round7.md) |
