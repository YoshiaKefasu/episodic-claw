...
| 指標 | 目標 |
|---|---|
| D1 新規生成 | 24h テスト中に **D1 ファイルが 0 件** 新規作成 |
| `file name too long` エラー | **0 件** (H-1 根本解消) |
| SleepTimer ログノイズ | `pebble: not found` のログ出力が **0 行/日** |
| 429 エラー後の embed 回復率 | HealingWorker が 429 失敗ファイルを **30 分以内に自動回復開始** |
