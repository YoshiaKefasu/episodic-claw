...
**Date:** 2026-04-09
**Current Version:** v0.4.1-hotfix
**Target Version:** v0.4.2 (収束完了)
**Status:** v0.4.2 収束完了（Cache DB / 64K分割 / 主要3経路cache化 / Continuity復元 / Backoff / Crash recovery / YAML剥がし / parsing/contract smoke）。small gap (≤50 msgs) は意図的例外。v0.4.3 で Full RPC integration 検討。
...
| 対象 | 理由 |
|---|---|
| D0/D1 二段構造 | 物語化で一段に統合。`Depth` フィールドは残すが、D1 consolidation パイプラインは廃止 (**v0.4.1 で完全排除予定** — [H-4](plans/v0.4.x/v0.4.1_post_release_hardening.md#h-4-d1-consolidation-の完全排除-high----h-1-の根本解決)) |
...
- [x] D0/D1 consolidation パイプラインの廃止（Legacy パスは維持 → **v0.4.1 H-4 で完全排除予定**）
- [x] CHANGELOG.md 更新
...
### Phase 4: 安定化 (v0.4.0) -- **Step 0/0b 完了**
**目標:** 本番デプロイ可能な状態

- [x] **Step 0:** デフォルトモデルを `openrouter/free` に統一 (worker / client のハードコード値)
- [x] **Step 0b:** `openclaw.plugin.json` 経由 `openrouterConfig` ネスト構造でユーザーカスタマイズ可能化 (`model`, `maxTokens`, `temperature`)
  - `maxTokens` はデフォルト省略 (OpenRouter 任せ)。ユーザーが必要時のみ設定
  - Post-Implementation Review: **Finding 0 件** (全 22 項目 PASS)
- [ ] 24 時間の本番運用テスト → **v0.4.1 に移行** (H-1 修正後に実施)
- [ ] エピソードの品質レビュー → **v0.4.1 に移行**
- [ ] Pool 蓄積効果の検証 (P2-F3 フォローアップ) → **v0.4.1 に移行**
- [ ] Recall 精度の比較 → **v0.4.1 に移行**
- [ ] ドキュメント更新（README、設定ガイド） → **v0.4.1 に移行**
- [ ] ClawHub パブリッシュ準備 → **v0.4.1 に移行**

> 詳細は [Phase 4 プラン](plans/v0.4.x/v0.4.0_phase4_stabilization_release.md) 参照。

**推定工数:** 2-3 時間 (コード変更完了、テストは v0.4.1 へ)

### v0.4.1: Post-Release Hardening -- **計画策定済み**
**目標:** D1 consolidation の完全排除 + Go sidecar バックグラウンド処理の堅牢化 + 24h テスト

| ID | 重要度 | 問題 | 対処 |
|---|---|---|---|
| H-4 | **HIGH** | D1 consolidation が物語化モードで不要 (H-1 の根本原因) | SleepTimer + RPC から呼び出し削除。D1 パイプライン排除 |
| H-1 | ~~CRITICAL~~ | D1 slug 長すぎでファイル作成失敗 | **H-4 に統合** (D1 排除で自動解消) |
| H-2 | LOW | SleepTimer `pebble:not found` ログノイズ | `not found` 時のログ出力抑制 |
| H-3 | MEDIUM | Embedding 429 大量発生 | バッチサイズ制限 + 429 バックオフ改善 + probe 復帰方式 |

**引き継ぎ:** Phase 4 残タスク (24h テスト、品質レビュー、Recall 比較) は H-4 修正後に v0.4.1 で実施。

> 詳細は [v0.4.1 プラン](plans/v0.4.x/v0.4.1_post_release_hardening.md) 参照。
...
```
Phase A (v0.4.0-v0.4.1): 物語化 + Invisible Footer + D1 排除
  └── ストレージは PebbleDB + go-hnsw + Bleve のまま
  └── store.go に StoreBackend インターフェースを導出（準備）
  └── 安定動作を最優先
  └── v0.4.1: D1 consolidation 完全排除 + HealingWorker embed バースト制御改善
...
## Pro Engineer Cross-Consistency Review -- 2026-04-09
...
> Scope: 3 ドキュメント間の整合性検証

### Current Reality (Source Code vs. Document)

| 項目 | ドキュメント記述 | コード現状 | 状態 |
|---|---|---|---|
| `openrouter-client.ts` DEFAULT_CONFIG | model: `openrouter/free` | L17: `model: "openrouter/free"` | MATCH |
| `config.ts` openrouterConfig ネスト解決 | ネスト優先 | L48: `rawConfig?.openrouterConfig?.model ??` | MATCH |
| `config.ts` maxTokens | 省略 = undefined | L49: 末尾にフォールバック値なし | MATCH |
| `config.ts` temperature | デフォルト 0.4 | L50: `?? 0.4` | MATCH |
| `narrative-worker.ts` model | `openrouter/free` | L99: `this.config.openrouterModel ?? "openrouter/free"` | MATCH |
| `frontmatter.go` Invisible Footer | Go sidecar が生成 | L182-186: `buf.WriteString("<!-- episodic-meta\n")` | MATCH |
| `main.go` SleepTimer | consolidation 呼び出しが存在 | L2290-2311: `vector.RunConsolidation` | MATCH (v0.4.1 で排除予定) |
| `main.go` HealingWorker | Pass 1-4 が必要 | L1515-1810: 4 Pass が実装されている | MATCH |
| `main.go` `pebble: not found` LOG | 毎 2 分 WARN | L2254: `EmitLog("[SleepTimer] WARN: ...")` | MATCH (問題確認) |

### Principle Filter

| Check | Result | Note |
|---|---|---|
| YAGNI — D1 consolidation 排除は必要か? | YES | 物語化で代替済み。二重要約は無駄 |
| KISS — もっとシンプルな方法? | noop 最簡 | noop レスポンス + 呼び出し削除が最小工数 |
| DRY — 重複排除? | N/A | D1 コードは dead code として残置。削除は YAGNI (将来の再利用可能性) |
| SOLID — 実際の問題? | なし | 安定化フェーズでは過度設計を避ける |

### Cross-Document 不整合の検出と修正完了一覧

| # | 不整合 | 対象ドキュメント | 修正内容 |
|---|---|---|---|
| C-1 | Phase 4 Status が `Not Started` のまま | Phase4.md L6 | `Step 0/0b 完了` に更新 |
| C-2 | Post-Release が v0.5.0 のみ言及、v0.4.1 が欠落 | Phase4.md L344-353 | v0.4.1 セクションを追加、v0.5.0 を「将来」に繰り下げ |
| C-3 | Section 2.4 D1 記述が「廃止」だが v0.4.1 参照なし | Roadmap L88 | v0.4.1 H-4 へのリンクを追加 |
| C-4 | Phase 3 D1 記述が `Legacy パスは維持` のまま | Roadmap L291 | v0.4.1 H-4 で完全排除予定を追記 |
| C-5 | Section 9 Phase A が v0.4.0 止まり | Roadmap L672 | v0.4.0-v0.4.1 に拡張、D1 排除を含む |
| C-6 | v0.4.1 プランに Parent リンクが欠落 | v0.4.1 L1 | Parent リンクを追加 |

### Pro Recommendation

> **v0.4.1 は H-4 (D1 排除) を最優先で実施し、その後 H-2 (15分) → H-3 (3 Phase) の順で進める。**
> D1 排除は変更箇所が 3 点のみでリスクが低く、H-1 のデータロスト根本原因を除去できる。
> consolidation.go 自体は dead code として残置し、**削除は v0.5.0 で判断する** (YAGNI: 今削除する理由がない)。

### Quick Wins (オプション問わず実施可能)

- [x] C-1: Phase4.md Status 更新
- [x] C-2: Phase4.md Post-Release に v0.4.1 セクション追加
- [x] C-3: Roadmap Section 2.4 に v0.4.1 H-4 リンク追加
- [x] C-4: Roadmap Phase 3 に v0.4.1 排除予定追記
- [x] C-5: Roadmap Section 9 Phase A を v0.4.0-v0.4.1 に拡張
- [ ] C-6: v0.4.1 プランに Parent リンク追加
