...
| [E3] LOW: 空文字列ガード | ✅ Resolved — `ErrEmptyEmbedInput` と provider / caller 両側の guard を 2026-03-29 に実装済み |
| [E2] LOW: フロントマター長 | ✅ Resolved — `frontmatter.Parse()` の `doc.Body` が frontmatter 除去後本文であることを 2026-03-29 に確認済み |
...
| LOW | [E3] 空文字列ガードを `EmbedContent` / `EmbedContentBatch` の入口に実装してチェックボックスを閉じる | 2026-03-29 実装済み。履歴保持のため記録のみ残す | No (R1 継続) |

---

## 11. 2026-03-29 フォローアップ

`plan_e2_e3_embed_guard.md` に沿って、Round 2 で残っていた LOW 2 件を解消した。

| ID | 2026-03-28 時点 | 2026-03-29 時点 |
|----|----------------|----------------|
| `E2` | 🔲 Open | ✅ Closed |
| `E3` | 🔲 Open | ✅ Closed |
...
- `frontmatter.Parse()` の `doc.Body` が frontmatter 除去後本文であることを確認
- `ErrEmptyEmbedInput` を追加し、`EmbedContent` / `EmbedContentBatch` は空入力を HTTP 前に reject
- `runAutoRebuild` `RunAsyncHealingWorker` `handleBatchIngest` `handleRecall` `handleSurprise` `processCluster` に caller 側 guard を追加

未完了:

- `go build ./...` は 2026-03-29 に Windows 上で成功確認済み。通常の `shell_command` からの `.exe` 起動が不安定だったため、`System.Diagnostics.ProcessStartInfo` 経由で実行した
