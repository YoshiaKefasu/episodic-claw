// Reasoning tag stripping for recall queries.
// Adapted from OpenClaw core (src/shared/text/code-regions.ts +
// src/shared/text/reasoning-tags.ts) — self-contained, zero external deps.
...
export function findCodeRegions(text: string): CodeRegion[] {
  ...
  for (const match of text.matchAll(inlineRe)) {
    ...
    const insideViable = regions.some((r) => start >= r.start && end <= r.end);
    if (!insideViable) {
